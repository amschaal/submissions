...
UCD_ACCOUNT_REGEXES = [
    ...
    "^([a-zA-Z0-9]{10})-([a-zA-Z0-9]{7})-([0-9]{6})-([a-zA-Z0-9]{6})[a-zA-Z0-9\-]*$"
]
...
class UCDPaymentSerializer(serializers.Serializer):
    ...
    def get_display(self, obj):
        return {
            "Payment Type": obj.get("payment_type", ""),
            "Payment Info": obj.get("payment_info", ""),
            "PPMS Order Ref #": obj.get("ppms_order_id", "")
        }
    ...
    def validate(self, data):
        ...
        PAYMENT_TYPES = [
            Submission.PAYMENT_UC,
            Submission.PAYMENT_WIRE_TRANSFER,
            Submission.PAYMENT_PO,
        ]
        payment_type = data.get("payment_type")
        payment_info = data.get("payment_info")
        if not payment_type:
            raise serializers.ValidationError(
                {"payment_type": "Please choose a payment type"}
            )
        if payment_type == Submission.PAYMENT_CREDIT_CARD and payment_info:
            raise serializers.ValidationError(
                {
                    "payment_info": "Do not enter anything into payment info when choosing credit card!"
                }
            )
            ...
            if not payment_info:
                raise serializers.ValidationError(
                    {
                        "payment_info": "Please enter the Aggie Enterprise account string that should be billed for this submission."
                    }
                )
            elif not self.validate_UCD_account(payment_info):
                raise serializers.ValidationError(
                    {
                        "payment_info": "The account format is invalid.  Please ensure that the format matches the format for Aggie Enterprise accounts (minimally XXXX-XXXXX-XXXXXXX-XXXXXX for CoA or XXXXXXXXXX-XXXXXXX-XXXXXX-XXXXXX for POET)."
                    }
                )
        elif (
            payment_type
            ...
            and not payment_info
        ):
            raise serializers.ValidationError(
                {"payment_info": "Please enter payment details."}
            )
        return data
